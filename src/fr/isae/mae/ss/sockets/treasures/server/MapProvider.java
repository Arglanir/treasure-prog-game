...
public class MapProvider {
	...
	public synchronized GameMap provideMap(String player, int currentGain) {
		/*
		 * Algorithm: try to find the map for the maximum min players with the current
		 * gain if waitingPlayers has enough players with the right level, start a new
		 * map with this player and other players, notify them with notifyAll()
		 * otherwise wait some time When awaken, if the awaited map is opened and we are
		 * the first in the waitingPlayers, then return it. Otherwise start the best map
		 * with the current waiting players
		 */
		...
		bestMap = null; int bestCrit = Integer.MIN_VALUE;
		for (WaitingPlayerInfo info: waitingPlayers) {
			...
			if (minPlayers > nbwaiting) continue;
		...
		for (String mapString : mapName2content.keySet()) {
			...
			int crit = 0;
			int mingain = Integer.parseInt(matcher.group(GROUP_MIN_GAIN)); // minimum level
	...
	private GameMap startMapWithPlayers(String player, String bestMap, List<String> otherPlayers) {
		...
		if (player != null) openedMap.addPlayer(player);
	...
	private String findMapMaxPlayersNoMax(int currentGain) {
		...
		for (String mapString : mapName2content.keySet()) {
			...
			if (Integer.parseInt(matcher.group(GROUP_MIN_GAIN)) > currentGain)
				continue;
			// if (Integer.parseInt(matcher.group(GROUP_MAX_GAIN)) < currentGain) continue;
			// ok, now we have a map with corresponding gain
